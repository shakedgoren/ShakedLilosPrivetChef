...
type Props = {
  ...
  iconOnly?: boolean;
  /**
   * הכפתור נשאר כמו שהוא גם בגרסת הראווה.
   *
   * ⚠ **למה זה נדרש · נמדד ב-24 בספטמבר 2026** · ההחלפה למטה
   * חלה על **כל** `ContinueButton`, כי כל כפתורי ההמשך הם המשך
   * של הזמנה. הכפתור ״שליחה״ בטופס השארת הפרטים הוא היוצא מן
   * הכלל — הוא לא ממשיך הזמנה אלא **שולח את הטופס**. בלי הדגל
   * הזה הוא קיבל את הכיתוב ״להזמנה״ ואת פעולת ה-CTA, כלומר
   * לחיצה על ״שליחה״ פתחה את הטופס מחדש ולא שלחה כלום.
   */
  keepInShowroom?: boolean;
...
export function ContinueButton({
   ...
   * הלחיצה יוצאת מהמסלול אל טופס השארת הפרטים.
   * ⚠ `keepInShowroom` מחריג · ראו התכונה בטיפוס.
   * ⚠ `iconOnly` מבוטל · כפתור חץ בלי מילים לא אומר ״להזמנה״.
   */
  if (SHOWROOM && !keepInShowroom) {
