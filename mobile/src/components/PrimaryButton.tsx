 ...
 *
 * ⚠ **גלולה מלאה · 26 בספטמבר 2026** · שקד: ״תן יותר רדיוס
 * עיגולי לכפתור התחברות עצמו כדי שהכפתור הנגרר ישב עליו
 * בקצוות בדיוק מוחלט״. 18 ⟵ `H / 2`.
 *
 * **למה דווקא `H / 2`** · הידית היא עיגול בקוטר 38 שיושב
 * ב-5 מהקצה, כלומר מרכזה 24 מקצה הכפתור ו-24 מלמעלה. קשת
 * הקצה של הכפתור מתעגלת סביב נקודה שנמצאת `RADIUS` מהקצה.
 * רק כש-`RADIUS` שווה ל-24 שתי הנקודות מתלכדות, ואז הידית
 * והקצה **קונצנטריים** — הפער ביניהם קבוע לכל אורך הקשת.
 * ב-18 הם לא היו, ולכן נראה שהיא ״נדחפת״ לפינה.
 */
const W = 168;
const H = 48;
const RADIUS = H / 2;
 ...
 * גודל הסמל בידית.
 *
 * ⚠ **16 ⟵ 24 · 26 בספטמבר 2026** · שקד שלחה תקריב של הידית
 * וכתבה ״הוא צריך לשבת ככה בדיוק״. הכיוון כבר היה נכון — מה
 * שהיה שונה זה הגודל. נמדד משתי התמונות, דיו לבן מול קוטר
 * העיגול:
 *
 *   אצל שקד   44% רוחב · 48% גובה
 *   אצלי      33%      · 32%
 *
 * הסמל יושב בתוך תיבת־אם עם אוויר משלו, ולכן `size` נומינלי
 * גדול מהדיו בפועל: 16 מתוך 38 (42%) נתנו 32% של דיו.
 */
const ARROW = 24;
...
const s = StyleSheet.create({
   ...
   */
  /**
   * ⚠ **היפוך אופקי · בקשת שקד, 26 בספטמבר 2026** · ״האייקון
   * בהתחברות הפוך, הכניסה צריכה להראות לתוך הכיוון של הגרירה
   * ולא ההפך״.
   *
   * הסמל `rectangle.portrait.and.arrow.forward` מצויר לשפה
   * שנקראת משמאל לימין: החץ נכנס לדלת **ימינה**. כאן גוררים
   * **שמאלה**, ולכן חץ שמצביע ימינה מצביע הפוך מהתנועה.
   * `scaleX: -1` מהפך את הציור, ואז החץ והגרירה באותו כיוון.
   */
  glyph: { transform: [{ scaleX: -1 }] },
  knob: {
    position: 'absolute',
    right: KNOB_INSET,
    top: KNOB_INSET,
