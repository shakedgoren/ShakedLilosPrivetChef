 ...
 */
/**
 * ⚠ **`lead` נוסף ב-17 בספטמבר 2026** · מאז שהמסך עצמו **עולה
 * מלמטה** בכניסה לקטגוריה (ראו `ScreenStage`), שורה שמתחילה לעלות
 * באותו רגע נבלעת בתנועה של המסך כולו. ההשהיה הקצרה נותנת למסך
 * לנחות, ורק אז הפריטים מופיעים אחד אחרי השני — בדיוק הסדר ששקד
 * ביקשה: ״המסך יעלה למעלה ושהפריטים יופיעו אחד אחרי השני בעלייה״.
 *
 * ⚠ **התקרה ירדה משמונה לשש** · עם ההשהיה, שמונה מדרגות דחקו את
 * השורה האחרונה מעבר לשנייה וחצי.
 */
export const STEP_IN = { ms: 480, stagger: 100, rise: 22, maxSteps: 6, lead: 260 } as const;
