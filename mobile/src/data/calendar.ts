...
/**
 * מארזים שנמסרים **בימי שישי בלבד**.
 *
 * ⚠ **בקשה של שקד (17 בספטמבר 2026)** · ״קחו כמה שבא לכם, חגיגה
 * בשולחן (סלטים, עיקרית), הכל עלינו — צריכים להיות מוגבלים בבחירת
 * תאריך רק לימי שישי״. שאר המארזים (חלה לכל אירוע, פרימיום, טעם
 * של שנה טובה) נשארים פתוחים בכל יום.
 */
export const fridayOnly = (k: string) => dowOf(k) === FRIDAY && k >= todayKey();

/** ⚠ נוסח שכתבתי · אין לו מקור בקנבס */
export const FRIDAY_HINT = 'המארז הזה נמסר בימי שישי בלבד';
